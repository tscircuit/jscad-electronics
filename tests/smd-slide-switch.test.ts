...
for (const footprint of [
  "smdslideswitch7",
  "smdslideswitch7_signalcols4_missing(2)_p1.5mm_mpx7.2mm",
  "smdslideswitch7_noholes",
]) {
  test(`${footprint} has three terminals, four mounting tabs, and requested pegs`, async () => {
    ...
    const caseBounds = jscad.measurements.measureBoundingBox(
      geometries[0]!.geom,
    )
    ...
    const knobBounds = jscad.measurements.measureBoundingBox(
      geometries[2]!.geom,
    )
