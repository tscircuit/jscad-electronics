...
test("SOIC: physical outline, pitch, seating plane and embedded leads", () => {
  for (const { props: p, footprint } of Object.values(soicVariants)) {
    expect(
      fp
        .string(footprint)
        .circuitJson()
    ...
    const solids = getComponentModel(SOIC, p).geometries.map((g) => g.geom)
