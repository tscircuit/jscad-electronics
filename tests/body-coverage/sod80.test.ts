...
test("sod80 body", async () => {
  const probe = probeFor("sod80")
  const height = nominalHeightFor("sod80")
  ...
  await expect(bottom).toMatchPngSnapshot(import.meta.path, "sod80-underside")
})
