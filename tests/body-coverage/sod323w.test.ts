...
test("sod323w body", async () => {
  const probe = probeFor("sod323w")
  const height = nominalHeightFor("sod323w")
  ...
  await expect(bottom).toMatchPngSnapshot(import.meta.path, "sod323w-underside")
})
