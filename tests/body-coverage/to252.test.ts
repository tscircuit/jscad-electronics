...
test("to252 body", async () => {
  const probe = probeFor("to252")
  const height = nominalHeightFor("to252")
  ...
  await expect(bottom).toMatchPngSnapshot(import.meta.path, "to252-underside")
})
