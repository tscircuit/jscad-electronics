...
test("sod882d body", async () => {
  const probe = probeFor("sod882d")
  const height = nominalHeightFor("sod882d")
  ...
  await expect(bottom).toMatchPngSnapshot(import.meta.path, "sod882d-underside")
})
