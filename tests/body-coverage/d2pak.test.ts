...
test("d2pak body", async () => {
  const probe = probeFor("d2pak")
  const height = nominalHeightFor("d2pak")
  ...
  await expect(bottom).toMatchPngSnapshot(import.meta.path, "d2pak-underside")
})
