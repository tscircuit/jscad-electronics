...
test("to263 body", async () => {
  const probe = probeFor("to263")
  const height = nominalHeightFor("to263")
  ...
  await expect(bottom).toMatchPngSnapshot(import.meta.path, "to263-underside")
})
