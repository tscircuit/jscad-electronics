...
test("sot25 body", async () => {
  const probe = probeFor("sot25")
  const height = nominalHeightFor("sot25")
  ...
  await expect(bottom).toMatchPngSnapshot(import.meta.path, "sot25-underside")
})
