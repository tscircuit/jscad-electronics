...
test("sod110 body", async () => {
  const probe = probeFor("sod110")
  const height = nominalHeightFor("sod110")
  ...
  await expect(bottom).toMatchPngSnapshot(import.meta.path, "sod110-underside")
})
