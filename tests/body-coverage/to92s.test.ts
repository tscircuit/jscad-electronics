...
test("to92s body", async () => {
  const probe = probeFor("to92s")
  const height = nominalHeightFor("to92s")
  ...
  await expect(bottom).toMatchPngSnapshot(import.meta.path, "to92s-underside")
})
