...
test("radial body", async () => {
  const probe = probeFor("radial")
  const height = nominalHeightFor("radial")
  ...
  await expect(bottom).toMatchPngSnapshot(import.meta.path, "radial-underside")
})
