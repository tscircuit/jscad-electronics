...
test("Footprinter3d ssop: six views", async () => {
  await expect(
    await renderComponentContactSheet(
      createElement(Footprinter3d, v["SSOP explicit package"]),
    ),
  ).toMatchPngSnapshot(import.meta.path, "footprinter3d-ssop-model")
}, 30000)
