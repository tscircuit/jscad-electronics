...
test("lga string body dimensions: six views", async () => {
  await expect(
    await renderComponentContactSheet(
      createElement(Footprinter3d, { footprint: bodyDimensionFootprints.LGA }),
    ),
  ).toMatchPngSnapshot(import.meta.path, "fp-lga-body-dimensions")
}, 30000)
