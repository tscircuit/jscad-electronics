...
export const Footprinter3d = ({ footprint }: { footprint: string }) => {
  ...
  switch (fpJson.fn) {
    ...
    case "qfn": {
      ...
      const isQfaa48 =
        fpJson.num_pins === 48 &&
        Math.abs((fpJson.thermalpad?.x ?? 0) - 4.6) < 0.02 &&
        Math.abs((fpJson.thermalpad?.y ?? 0) - 4.6) < 0.02
      const isRge24 =
        fpJson.num_pins === 24 &&
        (fpJson.thermalpad as unknown) === true &&
        Math.abs((fpJson.w ?? 0) - 5.08) < 0.02 &&
        Math.abs((fpJson.h ?? 0) - 5.1) < 0.02
      const isCp2102Qfn28 =
        fpJson.num_pins === 28 &&
        Math.abs((fpJson.thermalpad?.x ?? 0) - 3.2) < 0.02 &&
        Math.abs((fpJson.thermalpad?.y ?? 0) - 3.2) < 0.02
      if (isRhb32)
        return (
          <QFN
            num_pins={32}
    ...
    case "sot143":
      return <SOT143 footprint={normalizedFootprint} />
    case "sot223":
      return <SOT223 />
    case "sot23w":
      return <SOT23W />
